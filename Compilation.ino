...
#include <math.h>
#define MQTT_MAX_PACKET_SIZE 768   // Must precede PubSubClient.h
...
static const float PH_SLOPE      = -6.25f;
static const float PH_OFFSET     = 23.63f;
...
// ════════════════════════════════════════════════════════════════
//  WebServer — moved to WebServer.ino (dashboard + /data handler)
//  MQTT JSON builder — moved to Mqtt.ino (buildSensorJSON)
// ════════════════════════════════════════════════════════════════
...
void loop() {
  ...
  if (now - tMQTT >= MQTT_PUB_INTERVAL) {
    ...
    if (mqttClient.connected()) {
      ...
      mqttClient.publish(MQTT_TOPIC_DATA, buf, true);
    }
  }
}

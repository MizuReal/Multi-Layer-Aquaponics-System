...
// ════════════════════════════════════════════════════════════════
//  BH1750 — safe init helper (handles purple expansion board
//  slow voltage ramp; retries up to 3x with 500 ms gap)
// ════════════════════════════════════════════════════════════════

bool initBH1750() {
  Wire.begin(21, 22);
  Wire.setClock(50000);   // 50 kHz — stable with long jumper wires
  delay(500);             // purple expansion board: slow 3.3V ramp; BH1750 spec = 180 ms min
  for (int attempt = 1; attempt <= 3; attempt++) {
    bool ok = lightMeter.begin(BH1750::CONTINUOUS_HIGH_RES_MODE, 0x23);
    if (ok) {
      Serial.printf("BH1750: init OK (attempt %d)\n", attempt);
      return true;
    }
    Serial.printf("BH1750: init FAIL attempt %d — retrying\n", attempt);
    Wire.end();
    delay(500);
    Wire.begin(21, 22);
    Wire.setClock(50000);
  ...
  Serial.println("BH1750: all init attempts FAILED");
  return false;
}
...
void setup() {
  ...
  // ── BH1750 — init AFTER WiFi+GSM settled ─────────────────────
  // Purple expansion board has slow 3.3V ramp; initBH1750() handles
  // the 500 ms settling delay and retries up to 3x on failure.
  bh1750Ok = initBH1750();
...
void loop() {
  ...
  if (now - tLux >= 1200) {
    ...
    if (bh1750Ok) {
      ...
      if (lux < 0) {
        // -2 = I²C bus error — reinitialise and recover
        Serial.println("BH1750: read error (-2) — reinitialising");
        bh1750Ok = initBH1750();
        lastLux  = 0.0f;
      } else {
        lastLux = lux;
      ...
      // begin() failed at setup — keep retrying every 1200 ms cycle
      bh1750Ok = initBH1750();
      lastLux  = 0.0f;
    }
  ...
  if (now - tTemp >= 2000) {
    ...
    setTempRelay(waterTemp > TEMP_THRESHOLD);
  }
